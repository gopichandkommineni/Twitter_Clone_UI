...
channel.on('login_response', function (payload) {

  if (payload.message.search("Invalid") != -1 || payload.message.search("Passcode") != -1) {
...
channel.on('follow_response', function (payload) {
  list.append("<b>" + (payload.message || 'Celebrity') + "<br>");
  list.prop({ scrollTop: list.prop("scrollHeight") });
});

channel.on('register_response', function (payload) {
  alert(payload.message);
  //  list.append(`<b>${payload.message || 'Celebrity'}<br>`);
  // list.prop({scrollTop: list.prop("scrollHeight")});
});
